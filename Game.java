...
public class Game {
    ...
    public boolean endCheck() {
        ...
        try {
            farmer.checkCoins(5, true); // Throws NotEnoughMoneyException if not enough coins
            return false; // If checkCoins() doesn't throw an exception, player can still buy crops
        } catch (NotEnoughMoneyException e) {
            return !farm.cropFarmCheck(day); // If no money & no crops, game ends
        }
    }
     ...
     */
    public boolean displayEndInfo(Scanner scanner) throws InvalidInputException, ScannerInputException { // Temp UI
        System.out.println();
     ...
     * @param scanner the Scanner object used to detect input from the player.
     */
    public void choiceMenu(Scanner scanner) throws InvalidInputException, ScannerInputException {
            ...
            switch (choice) {
                case 'P': // PLOW action
                    if (!farm.plowedFarmCheck()) {
                        ...
                        throw new NoAvailablePlotException();
                    }
                    break;
                case 'T': // PLANT action
                    if (farm.plantFarmCheck()) {
                        ...
                        plotInput(scanner, xy);
                        if (farm.getPlot(xy.get(0), xy.get(1)).plantCheck() != 0) {
                            throw new PlotAlreadyHasCropException();
                        }
                ...
                case 'W': // WATER action
                    if (farm.cropFarmCheck(day)) {
                        ...
                        throw new NoCropInPlotException();
                    }
                    break;
                ...
                case 'X': // PICKAXE action
                    if (farm.rockFarmCheck()) {
                        farmer.checkCoins(50, false);
                        System.out.println("  Which plot to use pickaxe on?");
                        plotInput(scanner, xy);
                        farmer.pickaxe(farm.getPlot(xy.get(0), xy.get(1)));
                    } else {
                        throw new NoRockInPlotException();
                    }
                    break;
                case 'R': // REGISTER action
                    try {
                        farmer.checkRegisterEligibility(); // Throws InvalidInputException if not eligible
                        ...
                        if (confirm == 'Y') {
                            farmer.register(); // Throws NotEnoughMoneyException if not enough coins
                            System.out.println("  Registration successful! You are now a " + farmer.getType() + ".");
                        }
                    } catch (InvalidInputException | NotEnoughMoneyException e) {
                        System.out.println("  " + e.getMessage()); // Show the appropriate error message
                    }
                    break;
                case 'H': // HARVEST action
                    if (farm.harvestFarmCheck(day)) {
                        ...
                        throw new CropNotMatureException();
                    }
                    break;
     ...
     */
    public char charInput(Scanner scanner) throws InvalidInputException, ScannerInputException {
        char choice;
        do {
            System.out.print("  ");
            ...
            if (input.isEmpty()) {
                throw new ScannerInputException(); // Handle empty input
            }

            choice = input.charAt(0); // Take the first character

            if (!Character.isLetter(choice)) {
                throw new InvalidInputException(); // Ensure it's a letter
            }

        } while(!(choice >= 'a' && choice <= 'z') && !(choice >= 'A' && choice <= 'Z'));

        return Character.toUpperCase(choice); // Convert to uppercase before returning
    }
    ...
    /**
     * This function checks the money requirement and plants the crop if the user has enough objectCoins.
     * @param x
     * @param y
     * @param crop
     * @param cost
     ...
     */
    private void checkAndPlant(int x, int y, Crop crop, int cost) throws NotEnoughMoneyException {
        farmer.checkCoins(cost, true); // Throws NotEnoughMoneyException if not enough money
        farmer.plant(farm.getPlot(x, y), crop);
    }
    ...
    public void plantInput(Scanner scanner, int x, int y) {
        ...
        do {
            try {
                ...
                switch (choice) {
                    case 'T':
                        checkAndPlant(x, y, new Crop("Turnip", "Root", day, 2, 1, 2, 0, 1, 1, 2, 5, 6, 5), 5);
                        choice = 'E';
                        break;
                    case 'C':
                        checkAndPlant(x, y, new Crop("Carrot", "Root", day, 3, 1, 2, 0, 1, 1, 2, 10, 9, 7.5), 10);
                        choice = 'E';
                        break;
                    case 'P':
                        checkAndPlant(x, y, new Crop("Potato", "Root", day, 5, 3, 4, 1, 2, 1, 10, 20, 3, 12.5), 20);
                        choice = 'E';
                        break;
                    case 'R':
                        checkAndPlant(x, y, new Crop("Rose", "Flower", day, 1, 1, 2, 0, 1, 1, 1, 5, 5, 2.5), 5);
                        choice = 'E';
                        break;
                    case 'U':
                        checkAndPlant(x, y, new Crop("Turnips", "Flower", day, 2, 2, 3, 0, 1, 1, 1, 10, 9, 5), 10);
                        choice = 'E';
                        break;
                    case 'S':
                        checkAndPlant(x, y, new Crop("Sunflower", "Flower", day, 3, 2, 3, 1, 2, 1, 1, 20, 19, 7.5), 20);
                        choice = 'E';
                        break;
                    case 'M':
                        ...
                        checkAndPlant(x, y, new Crop("Mango", "Tree", day, 10, 7, 7, 4, 4, 5, 15, 100, 8, 25), 100);
                        choice = 'E';
                        break;
                    case 'A':
                        if (farm.adjacentPlotCheck(x, y)) {
                        ...
                        checkAndPlant(x, y, new Crop("Apple", "Tree", day, 10, 7, 7, 5, 5, 10, 15, 200, 5, 25), 200);
                        choice = 'E';
                        break;
                    case 'E':
                        break;
